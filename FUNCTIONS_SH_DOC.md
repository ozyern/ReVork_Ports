...
Related files:
- `functions.sh:1` (the file itself)
- `port.sh:44` (`source functions.sh`)
- `PORT_SH_DOC.md:1` (full flow documentation for `port.sh`)
...
- Dependency command existence checks (`check`)
- Image extraction (ext4/EROFS) and AVB disabling (`extract_partition` / `disable_avb_verify`)
...
- Feature XML and build.prop diff management (`add_feature_v2` / `remove_feature` / `add_prop_v2` and others)

Important:
- Written for bash (`#!/bin/bash`). On Windows, WSL or MSYS2/Git Bash is required.
...
The `error` / `yellow` / `blue` / `green` functions defined at `functions.sh:4` switch display language based on `$LANG` when called with 2 arguments.
...
So the standard calling convention in `port.sh` is to pass both `"Japanese text" "English text"` as two arguments.

### Functions: `error` / `yellow` / `blue` / `green`
- Defined at: `functions.sh:4` / `:21` / `:38` / `:55`
- Arguments:
  - 2 args: `(<Japanese>, <English>)`
  - 1 arg: `(<display string>)`
- Side effects: Prints to stdout with a timestamp and color (red / yellow / blue / green)
...
This compensates for behavioral differences in BSD tools (e.g. `sed -i`) on macOS.
...
Many external tools are required — if something fails, check your dependencies (`setup.sh`) and `PATH` first.
...
Use case:
- When applying a simple smali substitution to an app or framework file during `port.sh`'s patch phase.
...
- Purpose:
  - Spoofs the brand check in `SmartSideBar.apk` (e.g. the realme brand detection).
- Key steps:
...
## 6. Image Extraction and AVB Disabling
